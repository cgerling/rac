...
const TimeLine = ({className, duration = 0, time = 0, onSkip = () => {}}) => {
  ...
  return (
    <div className={cn('TimeLine__track', {}, className)}>
      ...
      >
        ...
        >
          <div
            className={cn('TimeLine__hover-notch', {
              inverted: hover.time < time,
            })}
          />
          <div className="TimeLine__hover-time">{format(hover.time)}</div>
        </div>
      </div>
      <div
        className="TimeLine__scrubbed"
        style={{width: toPercentage(time, duration)}}
      ></div>
    </div>
  )
}
